...
function sortHarchObjectAlph(object1, object2) {
    return determineAlphOrder(object1.Module, object2.Module);
}
...
function cleanJSON(data, filename, itemToAppend) {
    ...
    jsonData.data = jsonData.data.map(function (videoObject) {
        ...
        videoObject.Module = parseInt(metaData[1].replace(/0*(?=[1-9])/g, "")) - 1;
        return videoObject;
    });
...
(function () {
    ...
    condenseObjets("./scripts", function (data) {
        ...
        var courses = createHarchObject(consolidatedObject, "modules");
        ...
        var completeData = courses.map(function (course) {
            ...
            var courseList = consolidateByKey(course.modules, "Module");
            ...
            var weeks = createHarchObject(courseList, "videos").map(function (week) {
                ...
                return week;
            }).sort(function (week1, week2) {
                determineNumbericOrder(week1.week, week2)
            });
        ...
        writeToJSON(completeData.reduce(function (acc, courseObject) {
            ...
            acc[courseObject.Module] = courseObject.modules;
            return acc;
        }, {}));
    });
})();